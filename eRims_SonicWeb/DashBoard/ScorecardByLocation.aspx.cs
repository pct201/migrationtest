...
public partial class ScorecardByLocation : System.Web.UI.Page
{
    ...
    private string GetAggregatePerformance()
    {
        ...
        for (int i = 0; i < dtRegion.Rows.Count; i++)
        {
            ...
            strlink = Server.UrlEncode("JavaScript:OpenPopupForAggreage(\"" + dtRegion.Rows[i]["Region"].ToString() + "\",\"" + dtRegion.Rows[i]["DBA"].ToString().Replace("'", "&apos;") + "\",\"" + dtRegion.Rows[i]["Sonic_Location_Code"].ToString() + "\",\"" + Year.ToString() + "\",\"1\");").Replace("'", "%apos;");
    ...
    public string GetSLT()
    {
        ...
        if (Region == "Western Region")
        {
            ...
            dtReport.Merge(dsSCA.Tables[0]);
            dtReport.DefaultView.Sort = " Perfomance_Percentage_Score DESC, dba ASC";
            dtReport = dtReport.DefaultView.ToTable();
            decimal decAvg = 0;
        ...
        string strWidth = dsReport.Tables[0].Rows.Count > 25 ? "950" : "800";
        strChartXML.Append("<chart caption='Safety Leadership Team For Region " + Region + "' showalternatevgridcolor='0' bgColor='#FFFFFF' plotGradientColor='' xAxisName='Location' yAxisName='Level' useRoundEdges='0' showValues='0' formatNumberScale='0' showBorder='0' rotateYAxisName='0' showYAxisValues='0' yAxisMinValue='0' yAxisMaxValue='35' labelDisplay='ROTATE' maxColWidth='40' slantLabels='0' use3DLighting='1' divLineAlpha='0' baseFont='Verdana' baseFontColor='6f6c6c' baseFontSize='10'  trendValueFontBold ='1'>");
        ...
        for (int i = 0; i < dtReport.Rows.Count; i++)
        {
            ...
            strChartXML.AppendFormat("<set label='{0}' value='{1}' link='{2}' color='{3}'/>", dtReport.Rows[i][0].ToString().Replace("'", "&apos;"), dtReport.Rows[i][3].ToString(), strlink, Charts.GetColorCodeFromScore_SLT(Convert.ToDecimal(dtReport.Rows[i]["Perfomance_Percentage_Score"])));
        }
    ...
    public void GetTreadLineStyle_SLT(StringBuilder strChartXML, decimal Average)
    {
        ...
        // set Tread Lines
        strChartXML.Append("<trendLines>");
        strChartXML.Append("<line startValue='22.5' color='666666' displayvalue='" + Charts.Tin_Label_Graph + "' /> ");
        strChartXML.Append("<line startValue='24' color='666666' displayvalue='" + Charts.Bronze_Label_Graph + "' /> ");
        strChartXML.Append("<line startValue='27' color='666666' displayvalue='" + Charts.Silver_Label_Graph + "' /> ");
        strChartXML.Append("<line startValue='28.5' color='666666' displayvalue='" + Charts.Gold_Label_Graph + "' /> ");
        strChartXML.Append("<line startValue='30' color='666666' displayvalue='" + Charts.Platinum_Label_Graph + "' /> ");

        strChartXML.Append("<line startValue='29' color='FF0000' displayvalue='{br}Cross The Line' valueOnRight='1' thickness='2' /> ");
