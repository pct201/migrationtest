    ...
    public class cWorkerCompReserveWork : RIMS_Base.cWorkerCompReserveWork
    {
        ...
        public override int InsertUpdate_RWComp(RIMS_Base.cWorkerCompReserveWork obj_WorkCompRW)
        {
            ...
            try
            {
                ...
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_InsertUpdate");
                dbHelper.AddParameter(cmd, "@PK_Workers_Comp_RW_ID", DbType.Int32, ParameterDirection.InputOutput, "PK_Workers_Comp_RW_ID", DataRowVersion.Current, obj_WorkCompRW.PK_Workers_Comp_RW_ID);
                dbHelper.AddInParameter(cmd, "@FK_Workers_Comp", DbType.Decimal, obj_WorkCompRW.FK_Workers_Comp);
                ...
                nRetVal = int.Parse(dbHelper.GetParameterValue(cmd, "@PK_Workers_Comp_RW_ID").ToString());
                return nRetVal;
            }
            catch (Exception)
            {
        ...
        public override int Delete_RWComp(System.String lPK_Workers_Comp_RW_ID)
        {
            ...
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_Delete");
                dbHelper.AddInParameter(cmd, "@PK_Workers_Comp_RW_ID", DbType.String, lPK_Workers_Comp_RW_ID);
        ...
        public override List<RIMS_Base.cWorkerCompReserveWork> GetAll(Boolean blnIsActive)
        {
            ...
            try
            {
                ...
                List<RIMS_Base.cWorkerCompReserveWork> results = new List<RIMS_Base.cWorkerCompReserveWork>();
                while (reader.Read())
                {
                    cWorkerCompReserveWork obj_WorkCompRW = new cWorkerCompReserveWork();
        ...
        public override List<RIMS_Base.cWorkerCompReserveWork> Workers_Comp_RWRecords(System.Decimal Worker_Comp_ID)
        {
            ...
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_Records");
                dbHelper.AddInParameter(cmd, "@PK_Workers_Comp_ID", DbType.Decimal, Worker_Comp_ID);
                IDataReader reader = dbHelper.ExecuteReader(cmd);
                List<RIMS_Base.cWorkerCompReserveWork> results = new List<RIMS_Base.cWorkerCompReserveWork>();
                while (reader.Read())
                {
        ...
        public override List<RIMS_Base.cWorkerCompReserveWork> Workers_Comp_RWRecords_OutStand(decimal Worker_Comp_ID)
        {
            ...
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_Records_OutStand");
                dbHelper.AddInParameter(cmd, "@PK_Workers_Comp_ID", DbType.Decimal, Worker_Comp_ID);
                IDataReader reader = dbHelper.ExecuteReader(cmd);
                List<RIMS_Base.cWorkerCompReserveWork> results = new List<RIMS_Base.cWorkerCompReserveWork>();
                while (reader.Read())
        ...
        public override List<RIMS_Base.cWorkerCompReserveWork> Geters_Comp_RWByID(System.Decimal pK_Workers_Comp_RW_ID)
        {
            Database dbHelper = null;
            DbCommand cmd = null;
            try
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_Select");
                dbHelper.AddInParameter(cmd, "@PK_Workers_Comp_RW_ID", DbType.Decimal, pK_Workers_Comp_RW_ID);
        ...
        public override System.Data.DataSet GetPaymentRecords(System.Decimal Worker_Comp_ID)
        {
            ...
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("WorkerCompReserve_PaymentDetails");
                dbHelper.AddInParameter(cmd, "@FK_Workers_Comp_ID", DbType.Decimal, Worker_Comp_ID);
        ...
        protected RIMS_Base.Dal.cWorkerCompReserveWork MapFrom(IDataReader reader)
        {
            RIMS_Base.Dal.cWorkerCompReserveWork obj_WorkCompRW = new RIMS_Base.Dal.cWorkerCompReserveWork();
            if (!Convert.IsDBNull(reader["PK_Workers_Comp_RW_ID"])) { obj_WorkCompRW.PK_Workers_Comp_RW_ID = Convert.ToInt32(reader["PK_Workers_Comp_RW_ID"], CultureInfo.InvariantCulture); }
            if (!Convert.IsDBNull(reader["FK_Workers_Comp"])) { obj_WorkCompRW.FK_Workers_Comp = Convert.ToDecimal(reader["FK_Workers_Comp"], CultureInfo.InvariantCulture); }
        ...
        public override List<RIMS_Base.cWorkerCompReserveWork> GetClaimInfoByClaimNo(string m_strClaimNo)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    cWorkerCompReserveWork objk_Writing = new cWorkerCompReserveWork();
        ...
        public override DataSet GetWCReserveHistoryLabel()
        {
            DbCommand cmd = null;
            ...
            {
                dbHelper = DatabaseFactory.CreateDatabase();
                cmd = dbHelper.GetStoredProcCommand("Get_WCReserveHistory_Label");
        ...
        protected RIMS_Base.Dal.cWorkerCompReserveWork MapFromClaimInfo(IDataReader reader)
        {
            RIMS_Base.Dal.cWorkerCompReserveWork objk_Writing = new RIMS_Base.Dal.cWorkerCompReserveWork();
        ...
        protected RIMS_Base.Dal.cWorkerCompReserveWork MapFromWCClaimInfo(IDataReader reader)
        {
            RIMS_Base.Dal.cWorkerCompReserveWork objk_Writing = new RIMS_Base.Dal.cWorkerCompReserveWork();
            if (!Convert.IsDBNull(reader["tblName"])) { objk_Writing.TableName = Convert.ToString(reader["tblName"], CultureInfo.InvariantCulture); }
            if (!Convert.IsDBNull(reader["tblFK"])) { objk_Writing.TableFK = Convert.ToInt32(reader["tblFK"], CultureInfo.InvariantCulture); }
          ...
          //  if (!Convert.IsDBNull(reader["employee"])) { objk_Writing.Employee = Convert.ToString(reader["employee"], CultureInfo.InvariantCulture); }
            ...
            //  if (!Convert.IsDBNull(reader["FK_Claimant_id"])) { objk_Writing.Employee_Claimant = Convert.ToInt32(reader["FK_Employee_Claimant"], CultureInfo.InvariantCulture); }
            if (!Convert.IsDBNull(reader["INDEMNITY_outstanding"])) { objk_Writing.Outstanding_INDEMNITY = Convert.ToDecimal(reader["INDEMNITY_outstanding"], CultureInfo.InvariantCulture); }
            if (!Convert.IsDBNull(reader["Medical_outstanding"])) { objk_Writing.Outstanding_Medical = Convert.ToDecimal(reader["Medical_outstanding"], CultureInfo.InvariantCulture); }
            if (!Convert.IsDBNull(reader["Expense_outstanding"])) { objk_Writing.Outstanding_Expense = Convert.ToDecimal(reader["Expense_outstanding"], CultureInfo.InvariantCulture); }
            return objk_Writing;
