...
/// <summary>
/// Summary description for clsPrintManagementNotes
/// </summary>
public class clsPrintManagementNotes
{
	public clsPrintManagementNotes()
	{
		//
    ...
    public static void PrintSelectedNotes(string strPKs, decimal PK_Management, string strNote_Type)
    {
        DataTable dtNotes;

        if (!string.IsNullOrEmpty(strPKs) && strNote_Type == "Management")
            dtNotes = clsSonic_Management_Notes.SelectByIDs(strPKs).Tables[0];
        else if (strNote_Type == "Management")
            dtNotes = clsSonic_Management_Notes.SelectByFK_Management_WithPaging(PK_Management, 1, 1000, "Note_Date", "asc").Tables[0];
        else if (!string.IsNullOrEmpty(strPKs))// && strNote_Type == "Sonic"
            dtNotes = clsSonic_Management_Notes.SelectByIDs(strPKs).Tables[0];
        else// if (strNote_Type == "ACI")
            dtNotes = clsSonic_Management_Notes.SelectByFK_Management(PK_Management).Tables[0];
        ...
        //Response.End();
        doc.Save(HttpContext.Current.Response, "ManagementNotes.doc", ContentDisposition.Attachment, Aspose.Words.Saving.SaveOptions.CreateSaveOptions(SaveFormat.Doc));
