...
{
	/// <summary>
	/// Data access class for SLT_BT_Security_Walk table.
	/// </summary>
	public sealed class clsSLT_BT_Security_Walk
	{

		#region Private variables used to hold the property values

		private decimal? _PK_SLT_BT_Security_Walk;
		private decimal? _FK_SLT_Meeting;
		private bool? _BT_Security_Walk_Comp;
		private DateTime? _BT_Security_Walk_Comp_Date;
		...
		/// <summary>
		/// Gets or sets the PK_SLT_BT_Security_Walk value.
		/// </summary>
		public decimal? PK_SLT_BT_Security_Walk
		{
			get { return _PK_SLT_BT_Security_Walk; }
			set { _PK_SLT_BT_Security_Walk = value; }
		}
		...
		/// <summary>
		/// Gets or sets the BT_Security_Walk_Comp value.
		/// </summary>
		public bool? BT_Security_Walk_Comp
		{
			get { return _BT_Security_Walk_Comp; }
			set { _BT_Security_Walk_Comp = value; }
		}

		/// <summary>
		/// Gets or sets the BT_Security_Walk_Comp_Date value.
		/// </summary>
		public DateTime? BT_Security_Walk_Comp_Date
		{
			get { return _BT_Security_Walk_Comp_Date; }
			set { _BT_Security_Walk_Comp_Date = value; }
		}
		...
		/// <summary>
		/// Initializes a new instance of the clsSLT_BT_Security_Walk class with default value.
		/// </summary>
		public clsSLT_BT_Security_Walk() 
		{
		...
		/// <summary>
		/// Initializes a new instance of the clsSLT_BT_Security_Walk class based on Primary Key.
		/// </summary>
		public clsSLT_BT_Security_Walk(decimal pK_SLT_BT_Security_Walk) 
		{
			DataTable dtSLT_BT_Security_Walk = SelectByPK(pK_SLT_BT_Security_Walk).Tables[0];

			if (dtSLT_BT_Security_Walk.Rows.Count == 1)
			{
				 SetValue(dtSLT_BT_Security_Walk.Rows[0]);

			}

		}

        public clsSLT_BT_Security_Walk(decimal FK_SLT_Meeting_Schedule, bool Status)
        {
            DataTable dtSLT_BT_Security_Walk = SelectBYFK(FK_SLT_Meeting_Schedule).Tables[0];
		...
		/// <summary>
		/// Initializes a new instance of the clsSLT_BT_Security_Walk class based on Datarow passed.
		/// </summary>
		private void SetValue (DataRow drSLT_BT_Security_Walk) 
		{
				if (drSLT_BT_Security_Walk["PK_SLT_BT_Security_Walk"] == DBNull.Value)
					this._PK_SLT_BT_Security_Walk = null;
				else
					this._PK_SLT_BT_Security_Walk = (decimal?)drSLT_BT_Security_Walk["PK_SLT_BT_Security_Walk"];

				if (drSLT_BT_Security_Walk["FK_SLT_Meeting"] == DBNull.Value)
					this._FK_SLT_Meeting = null;
				else
					this._FK_SLT_Meeting = (decimal?)drSLT_BT_Security_Walk["FK_SLT_Meeting"];

				if (drSLT_BT_Security_Walk["BT_Security_Walk_Comp"] == DBNull.Value)
					this._BT_Security_Walk_Comp = null;
				else
					this._BT_Security_Walk_Comp = (bool?)drSLT_BT_Security_Walk["BT_Security_Walk_Comp"];

				if (drSLT_BT_Security_Walk["BT_Security_Walk_Comp_Date"] == DBNull.Value)
					this._BT_Security_Walk_Comp_Date = null;
				else
					this._BT_Security_Walk_Comp_Date = (DateTime?)drSLT_BT_Security_Walk["BT_Security_Walk_Comp_Date"];

				if (drSLT_BT_Security_Walk["Sales_Reviewed"] == DBNull.Value)
					this._Sales_Reviewed = null;
				else
					this._Sales_Reviewed = (string)drSLT_BT_Security_Walk["Sales_Reviewed"];

				if (drSLT_BT_Security_Walk["Sales_Deficiencies"] == DBNull.Value)
					this._Sales_Deficiencies = null;
				else
					this._Sales_Deficiencies = (string)drSLT_BT_Security_Walk["Sales_Deficiencies"];

				if (drSLT_BT_Security_Walk["Sales_Comments"] == DBNull.Value)
					this._Sales_Comments = null;
				else
					this._Sales_Comments = (string)drSLT_BT_Security_Walk["Sales_Comments"];

				if (drSLT_BT_Security_Walk["Parts_Reviewed"] == DBNull.Value)
					this._Parts_Reviewed = null;
				else
					this._Parts_Reviewed = (string)drSLT_BT_Security_Walk["Parts_Reviewed"];

				if (drSLT_BT_Security_Walk["Parts_Deficiencies"] == DBNull.Value)
					this._Parts_Deficiencies = null;
				else
					this._Parts_Deficiencies = (string)drSLT_BT_Security_Walk["Parts_Deficiencies"];

				if (drSLT_BT_Security_Walk["Parts_Comments"] == DBNull.Value)
					this._Parts_Comments = null;
				else
					this._Parts_Comments = (string)drSLT_BT_Security_Walk["Parts_Comments"];

				if (drSLT_BT_Security_Walk["Service_Facility_Reviewed"] == DBNull.Value)
					this._Service_Facility_Reviewed = null;
				else
					this._Service_Facility_Reviewed = (string)drSLT_BT_Security_Walk["Service_Facility_Reviewed"];

				if (drSLT_BT_Security_Walk["Service_Deficiencies"] == DBNull.Value)
					this._Service_Deficiencies = null;
				else
					this._Service_Deficiencies = (string)drSLT_BT_Security_Walk["Service_Deficiencies"];

				if (drSLT_BT_Security_Walk["Service_Comments"] == DBNull.Value)
					this._Service_Comments = null;
				else
					this._Service_Comments = (string)drSLT_BT_Security_Walk["Service_Comments"];

				if (drSLT_BT_Security_Walk["Body_Shop_Reviewed"] == DBNull.Value)
					this._Body_Shop_Reviewed = null;
				else
					this._Body_Shop_Reviewed = (string)drSLT_BT_Security_Walk["Body_Shop_Reviewed"];

				if (drSLT_BT_Security_Walk["Body_Shop_Deficiencies"] == DBNull.Value)
					this._Body_Shop_Deficiencies = null;
				else
					this._Body_Shop_Deficiencies = (string)drSLT_BT_Security_Walk["Body_Shop_Deficiencies"];

				if (drSLT_BT_Security_Walk["Body_Shop_Comments"] == DBNull.Value)
					this._Body_Shop_Comments = null;
				else
					this._Body_Shop_Comments = (string)drSLT_BT_Security_Walk["Body_Shop_Comments"];

				if (drSLT_BT_Security_Walk["Bus_Off_Reviewed"] == DBNull.Value)
					this._Bus_Off_Reviewed = null;
				else
					this._Bus_Off_Reviewed = (string)drSLT_BT_Security_Walk["Bus_Off_Reviewed"];

				if (drSLT_BT_Security_Walk["Bus_Off_Deficiencies"] == DBNull.Value)
					this._Bus_Off_Deficiencies = null;
				else
					this._Bus_Off_Deficiencies = (string)drSLT_BT_Security_Walk["Bus_Off_Deficiencies"];

				if (drSLT_BT_Security_Walk["Bus_Off_Comments"] == DBNull.Value)
					this._Bus_Off_Comments = null;
				else
					this._Bus_Off_Comments = (string)drSLT_BT_Security_Walk["Bus_Off_Comments"];

				if (drSLT_BT_Security_Walk["Detail_Area_Reviewed"] == DBNull.Value)
					this._Detail_Area_Reviewed = null;
				else
					this._Detail_Area_Reviewed = (string)drSLT_BT_Security_Walk["Detail_Area_Reviewed"];

				if (drSLT_BT_Security_Walk["Detail_Deficiencies"] == DBNull.Value)
					this._Detail_Deficiencies = null;
				else
					this._Detail_Deficiencies = (string)drSLT_BT_Security_Walk["Detail_Deficiencies"];

				if (drSLT_BT_Security_Walk["Detail_Comments"] == DBNull.Value)
					this._Detail_Comments = null;
				else
					this._Detail_Comments = (string)drSLT_BT_Security_Walk["Detail_Comments"];

				if (drSLT_BT_Security_Walk["Car_Wash_Reviewed"] == DBNull.Value)
					this._Car_Wash_Reviewed = null;
				else
					this._Car_Wash_Reviewed = (string)drSLT_BT_Security_Walk["Car_Wash_Reviewed"];

				if (drSLT_BT_Security_Walk["Car_Wash_Deficiencies"] == DBNull.Value)
					this._Car_Wash_Deficiencies = null;
				else
					this._Car_Wash_Deficiencies = (string)drSLT_BT_Security_Walk["Car_Wash_Deficiencies"];

				if (drSLT_BT_Security_Walk["Car_Wash_Comments"] == DBNull.Value)
					this._Car_Wash_Comments = null;
				else
					this._Car_Wash_Comments = (string)drSLT_BT_Security_Walk["Car_Wash_Comments"];

				if (drSLT_BT_Security_Walk["Parking_Lot_Reviewed"] == DBNull.Value)
					this._Parking_Lot_Reviewed = null;
				else
					this._Parking_Lot_Reviewed = (string)drSLT_BT_Security_Walk["Parking_Lot_Reviewed"];

				if (drSLT_BT_Security_Walk["Parking_Deficiencies"] == DBNull.Value)
					this._Parking_Deficiencies = null;
				else
					this._Parking_Deficiencies = (string)drSLT_BT_Security_Walk["Parking_Deficiencies"];

				if (drSLT_BT_Security_Walk["Parking_Comments"] == DBNull.Value)
					this._Parking_Comments = null;
				else
					this._Parking_Comments = (string)drSLT_BT_Security_Walk["Parking_Comments"];

				if (drSLT_BT_Security_Walk["Updated_By"] == DBNull.Value)
					this._Updated_By = null;
				else
					this._Updated_By = (string)drSLT_BT_Security_Walk["Updated_By"];

				if (drSLT_BT_Security_Walk["Update_Date"] == DBNull.Value)
					this._Update_Date = null;
				else
					this._Update_Date = (DateTime?)drSLT_BT_Security_Walk["Update_Date"];

				if (drSLT_BT_Security_Walk["UniqueVal"] == DBNull.Value)
					this._UniqueVal = null;
				else
					this._UniqueVal = (string)drSLT_BT_Security_Walk["UniqueVal"];

				if (drSLT_BT_Security_Walk["FK_SLT_Meeting_Schedule"] == DBNull.Value)
					this._FK_SLT_Meeting_Schedule = null;
				else
					this._FK_SLT_Meeting_Schedule = (decimal?)drSLT_BT_Security_Walk["FK_SLT_Meeting_Schedule"];

				if (drSLT_BT_Security_Walk["Created_Date"] == DBNull.Value)
					this._Created_Date = null;
				else
					this._Created_Date = (DateTime?)drSLT_BT_Security_Walk["Created_Date"];
		...
		/// <summary>
		/// Inserts a record into the SLT_BT_Security_Walk table.
		/// </summary>
		/// <returns></returns>
		public int Insert()
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetStoredProcCommand("SLT_BT_Security_WalkInsert");

			
			db.AddInParameter(dbCommand, "FK_SLT_Meeting", DbType.Decimal, this._FK_SLT_Meeting);
			
			db.AddInParameter(dbCommand, "BT_Security_Walk_Comp", DbType.Boolean, this._BT_Security_Walk_Comp);
			
			db.AddInParameter(dbCommand, "BT_Security_Walk_Comp_Date", DbType.DateTime, this._BT_Security_Walk_Comp_Date);
		...
		/// <summary>
		/// Selects a single record from the SLT_BT_Security_Walk table by a primary key.
		/// </summary>
		/// <returns>DataSet</returns>
		private DataSet SelectByPK(decimal pK_SLT_BT_Security_Walk)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetStoredProcCommand("SLT_BT_Security_WalkSelectByPK");
		...
		/// <summary>
		/// Selects all records from the SLT_BT_Security_Walk table.
		/// </summary>
		/// <returns>DataSet</returns>
		public static DataSet SelectAll()
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetStoredProcCommand("SLT_BT_Security_WalkSelectAll");

			return db.ExecuteDataSet(dbCommand);
		}

		/// <summary>
		/// Updates a record in the SLT_BT_Security_Walk table.
		/// </summary>
		public void Update()
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetStoredProcCommand("SLT_BT_Security_WalkUpdate");

			
			db.AddInParameter(dbCommand, "PK_SLT_BT_Security_Walk", DbType.Decimal, this._PK_SLT_BT_Security_Walk);
		...
		/// <summary>
		/// Deletes a record from the SLT_BT_Security_Walk table by a composite primary key.
		/// </summary>
		public static void DeleteByPK(decimal pK_SLT_BT_Security_Walk)
		{
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetStoredProcCommand("SLT_BT_Security_WalkDeleteByPK");
        ...
        {
            Database db = DatabaseFactory.CreateDatabase();
            DbCommand dbCommand = db.GetStoredProcCommand("SelectSLT_BT_Security_WalkBYFK");
