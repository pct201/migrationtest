...
/// </summary>
public class clsPrintClaimNotesAP
{
    public clsPrintClaimNotesAP()
	{
		//
    ...
    public static void PrintSelectedSonicNotes(string strPKs, string FK_Table_Name, long ClaimID)
    {
        ...
        if (FK_Table_Name.ToLower() == clsGeneral.AP_Tables.AP_AL_FROIs.ToString().ToLower())
            dtClaim = clsAP_AL_FROIs.GetAL_FROIs_DetailByPK_AP_AL_FROIs(ClaimID).Tables[0];
        else if (FK_Table_Name.ToLower() == clsGeneral.AP_Tables.AP_DPD_FROIs.ToString().ToLower())
            dtClaim = clsAP_DPD_FROIs.GetDPD_FROIs_DetailByPK_AP_DPD_FROIs(ClaimID).Tables[0];
        

        DataTable dtNotes = clsAP_Notes.SelectByIDs(strPKs).Tables[0];
        ...
        doc.Save(HttpContext.Current.Response, "AssetProtectionSonicNotes.doc", ContentDisposition.Attachment, Aspose.Words.Saving.SaveOptions.CreateSaveOptions(SaveFormat.Doc));
