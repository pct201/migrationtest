...
using ERIMS.DAL;
/// <summary>
/// Summary description for clsPrintVideoRequestNotes
/// </summary>
public class clsPrintVideoRequestNotes
{
    public clsPrintVideoRequestNotes()
	{
		//
    ...
    public static void PrintSelectedNotes(string strPKs, decimal PK_Video_Request_Notes, string strNote_Type)
    {
        DataTable dtNotes;

        if (!string.IsNullOrEmpty(strPKs) && strNote_Type == "Video_Request")
            dtNotes = clsVideo_Request_Notes.SelectByIDs(strPKs).Tables[0];
        else if (strNote_Type == "Video_Request")
            dtNotes = clsVideo_Request_Notes.SelectByFK_Event_Video_Tracking_Request_WithPaging(PK_Video_Request_Notes, 1, 1000, "Note_Date", "asc").Tables[0];
        else if (!string.IsNullOrEmpty(strPKs))// && strNote_Type == "Sonic"
            dtNotes = clsVideo_Request_Notes.SelectByIDs(strPKs).Tables[0];
        else// if (strNote_Type == "ACI")
            dtNotes = clsVideo_Request_Notes.SelectByFK_Event_Video_Tracking_Request(PK_Video_Request_Notes).Tables[0];
        ...
        sbHTML.Append("<b>eRIMS2 - Selected Notes</b>");
        ...
        //Response.End();
        doc.Save(HttpContext.Current.Response, "VideoRequestNotes.doc", ContentDisposition.Attachment, Aspose.Words.Saving.SaveOptions.CreateSaveOptions(SaveFormat.Doc));
