...
public partial class WorkerCompensation_WorkerCompensationReserve : System.Web.UI.Page
{
	...
	{
        Response.Redirect("~/Workers_Compensation/Carrier.aspx");
	}
	...
	protected void lbtnOutstanding_Click(object sender, EventArgs e)
	{
        ...
        if (lstWorkerRW.Count > 0)
        {
            ...
            lblOSTotal.Text = lstWorkerRW[0].Total.ToString();
        }
    ...
    private List<RIMS_Base.cWorkerCompReserveWork> BindWorkerRW_OutStand()
    {
        try
        {
            m_objWorkerRW = new RIMS_Base.Biz.cWorkerCompReserveWork();
            lstWorkerRW = new List<RIMS_Base.cWorkerCompReserveWork>();
            //lstWorkerRW = m_objWorkerRW.GetAll(true);
            lstWorkerRW = m_objWorkerRW.Workers_Comp_RWRecords_OutStand(Convert.ToDecimal(Session["WorkerCompID"].ToString()));
	...
	private void BindClaimData()
	{
		try
		{
			...
			lblIncidentDt.Text = Convert.ToDateTime(lstClaimReservesInfo[0].IncidentDate).ToShortDateString();
...
//            rbtnEmployee.Items[0].Selected = true;
	...
	private void BindClaimAndReservesData()
	{
		try
		{
			...
			lstClaimReservesInfo = m_objClaimReservesInfo.GetWCClaimInfoByClaimNo(Claim_No);
			BindClaimData();
		}
		catch (Exception ex)
		{
	...
	private void ViewAllRecords()
	{
        ...
        if (lstWorkerRW.Count > 0)
        {
            lstAttachment = m_objAttachment.GetAllAttachmentGroup(AttachmentID, "Workers_Comp_RW");
        }
        else
        {
            lstAttachment = m_objAttachment.GetAllAttachmentGroup("0", "Workers_Comp_RW");
        }
