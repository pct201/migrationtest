...
public partial class SONIC_SLTSafetyWalk_YearlyQuestions : clsBasePage
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (dtMonthlyQuestions.Rows.Count > 0)
            Response.Redirect("SLT_Safety_Walk_Management_Annual_Grid.aspx?Year=" + Encryption.Encrypt(Convert.ToString(Tmp_Year)) , true);
        else
            Response.Redirect("SLT_Safety_Walk_Management_Annual.aspx?id=" + Encryption.Encrypt("0") + "&Year=" + Encryption.Encrypt(Convert.ToString(Tmp_Year)), true);
    }
    #endregion
}