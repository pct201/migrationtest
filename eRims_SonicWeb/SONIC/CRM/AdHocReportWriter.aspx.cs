...
public partial class SONIC_CRM_AdHocReportWriter : clsBasePage
{
    #region "ENUM"
    //Report Output Type
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if (blnHTML2Excel)
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename=\"" + "CRM Ad-Hoc Report.xlsx" + "\""));
    ...
    protected void lnkSendEmail_Click(object sender, EventArgs e)
    {
        ...
        if (blnHTML2Excel)
        {
            if (clsGeneral.SendAdHocReport("Ad Hoc Report", outputFiles, "CRM Ad-Hoc Report.xlsx", Convert.ToDecimal(ddlRecipientList.SelectedItem.Value)))
            {
    ...
    {
        DataTable dtAdHocReport = CRM_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            CRM_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<CRM_AdHocFilter> lstFilter = new List<CRM_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new CRM_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            CRM_AdHocReport ObjAdHocReport = new CRM_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<CRM_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = CRM_AdHocReport.SelectReportName().Tables[0];
    ...
    private void MoveListBoxItems(ListBox lstFrom, ListBox lstTo, bool OnlySelected, bool IsSelect)
    {
        ...
        {
            lstTo.Items.Clear();
            lstTo.DataSource = CRM_AdhocReportFields.GetAdHocFilterFields("O", rdbIncidentType.SelectedValue);
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<CRM_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new CRM_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    ...
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "CRM");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = CRM_AdhocReportFields.GetAdHocFilterFields("F", rdbIncidentType.SelectedValue);
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = CRM_AdhocReportFields.GetAdHocFilterFields("O", rdbIncidentType.SelectedValue);
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new CRM_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            if (rdbIncidentType.SelectedIndex == 0)
                ...
                Reader = AdHocReportHelper.GetAdHocReportCRMNonCustomer(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
    ...
    {
        CRM_AdHocReport objAdHocReport = new CRM_AdHocReport();
        ...
        {
            // Delete 
            CRM_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<CRM_AdhocReportFields> lstAdhoc = new List<CRM_AdhocReportFields>();
    ...
    {
        CRM_AdHocFilter objFilter = new CRM_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(CRM_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    private void LoadFilterControlDropDown(string Field_Header, string ConditionValue, ListBox lst_F)
    {
        ...
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "CRM");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(CRM_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
