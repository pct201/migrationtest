...
public partial class DPD_AdHocReportWriter : clsBasePage
{
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if (blnHTML2Excel) 
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename=\"" + "DPD Ad-Hoc Report.xlsx" + "\""));
    ...
    protected void lnkSendEmail_Click(object sender, EventArgs e)
    {
        ...
        if (blnHTML2Excel)
        {
            if (clsGeneral.SendAdHocReport("Ad Hoc Report", outputFiles, "DPD Ad-Hoc Report.xlsx", Convert.ToDecimal(ddlRecipientList.SelectedItem.Value)))
            {
    ...
    {
        DataTable dtAdHocReport = DPD_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            DPD_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<DPD_AdHocFilter> lstFilter = new List<DPD_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new DPD_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            DPD_AdHocReport ObjAdHocReport = new DPD_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<DPD_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = DPD_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<DPD_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new DPD_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    ...
                    //}
                    if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "legal entity")
                    {
                    ...
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "DPD");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = DPD_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = DPD_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new DPD_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            Reader = AdHocReportHelper.GetAdHocReportDPD(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
            ...
            if (Reader.Read())
            {
                ...
                {
                    sbRecord.Append("<br />");
                    sbRecord.Append("<b>Report Title : DPD Ad-Hoc Report </b>");
                    ...
                    if (!string.IsNullOrEmpty(strFirstGroupBy))
                    {
                        ...
                        if (strGroupByValue_1 != Convert.ToString(Reader[strFirstGroupBy]))
                        {
                            ...
                            if (strFormatFirstGroupBy == "decimal")
                                sbRecord.Append("<tr><td style='font-weight: bold;' align='right' colspan='" + (colcount) + "'>" + strFirstGroupBy + ": " + string.Format("{0:c2}", strGroupByValue_1) + "</td></tr>");
                            else if (strFormatFirstGroupBy == "datetime")
                            {
                                // it display only Time
                                if (strFirstGroupBy == "Time Theft Reported")
                                    sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>" + strFirstGroupBy + ": " + string.Format("{0:HH:mm}", Reader[strFirstGroupBy]) + "</td></tr>");
                                else sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>" + strFirstGroupBy + ": " + clsGeneral.FormatDBNullDateToDisplay(strGroupByValue_1) + "</td></tr>");
                            }
                            else sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>&nbsp;" + strFirstGroupBy + ": " + strGroupByValue_1 + "</td></tr>");
                        ...
                        else if ((Reader[strFirstGroupBy] == DBNull.Value || string.IsNullOrEmpty(Convert.ToString(Reader[strFirstGroupBy]))) && strNOGroup1 == string.Empty)
                        {
                            ...
                            sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "' >&nbsp;" + strFirstGroupBy + ": " + strNOGroup1 + "</td></tr>");
                    ...
                    if (!string.IsNullOrEmpty(strSecGroupBy))
                    {
                        ...
                        if (strGroupByValue_2 != Convert.ToString(Reader[strSecGroupBy]))
                        {
                            ...
                            if (strFormatSecGroupBy == "decimal")
                                sbRecord.Append("<tr><td style='font-weight: bold;' align='right' colspan='" + (colcount) + "'>" + strSecGroupBy + ": " + string.Format("{0:c2}", strGroupByValue_2) + "</td></tr>");
                            else if (strFormatSecGroupBy == "datetime")
                            {
                                // it display only Time
                                if (strSecGroupBy == "Time Theft Reported")
                                    sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>" + strSecGroupBy + ": " + string.Format("{0:HH:mm}", Reader[strSecGroupBy]) + "</td></tr>");
                                else sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>" + strSecGroupBy + ": " + clsGeneral.FormatDBNullDateToDisplay(strGroupByValue_2) + "</td></tr>");
                            }
                            else sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "'>&nbsp;" + strSecGroupBy + ": " + strGroupByValue_2 + "</td></tr>");
                        }
                        else if (Reader[strSecGroupBy] == DBNull.Value && strNOGroup2 == string.Empty)
                        {
                            strNOGroup2 = "No " + strSecGroupBy;
                            sbRecord.Append("<tr><td style='font-weight: bold;' colspan='" + (colcount) + "' >&nbsp;" + strSecGroupBy + ": " + strNOGroup2 + "</td></tr>");
                            //No Group by assign
                    ...
                    if (IsAllNull == false)
                    {
                        ...
                        for (int intColumn = 0; intColumn < Reader.FieldCount; intColumn++)
                        {
                            ...
                            if (strFirstGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) && strSecGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]))
                            {
                                ...
                                {
                                    if (Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) == "First Report Number" || Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) == "Year")
                                    {
    ...
    {
        DPD_AdHocReport objAdHocReport = new DPD_AdHocReport();
        ...
        {
            // Delete 
            DPD_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<DPD_AdhocReportFields> lstAdhoc = new List<DPD_AdhocReportFields>();
    ...
    {
        DPD_AdHocFilter objFilter = new DPD_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(DPD_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    /// </summary>
    private void LoadFilterControlAmount(DPD_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
