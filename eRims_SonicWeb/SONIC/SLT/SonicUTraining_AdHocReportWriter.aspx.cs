...
public partial class SONIC_SLT_SonicUTraining_AdHocReportWriter : clsBasePage
{
    ...
    {
        DataTable dtAdHocReport = Sonic_U_Train_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            Sonic_U_Train_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<Sonic_U_Train_AdHocFilter> lstFilter = new List<Sonic_U_Train_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new Sonic_U_Train_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            Sonic_U_Train_AdHocReport ObjAdHocReport = new Sonic_U_Train_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<Sonic_U_Train_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = Sonic_U_Train_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        if (decSelectedValue > 0)
            lstAdHoc = new Sonic_U_Train_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "code")
                    {
                        ComboHelper.FillCode(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "dealership")
                    {
                        ComboHelper.FillDealershipName(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "department")
                    {
                        ComboHelper.FillSonicUTrainDepartment(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "learning program")
                    {
                        ComboHelper.FillSonicUTrainLearningProgram(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "asset")
                    {
                        ComboHelper.FillSonicUTrainLearningAsset(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "learning program status")
                    {
                        ComboHelper.FillSonicUTrainLearningProgramStatus(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "asset status")
                    {
    ...
    protected void drpAmount_Changed(bool isDollarSign, DropDownList drpAmount_F, Label lblAmountText1_F, TextBox txtAmount1_F, Label lblAmountText2_F, TextBox txtAmount2_F, CompareValidator cvAmount)
    {
        ...
        if (!isDollarSign)
        {
            ...
            txtAmount2_F.MaxLength = txtAmount1_F.MaxLength = 17;
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = Sonic_U_Train_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = Sonic_U_Train_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new Sonic_U_Train_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            Reader = AdHocReportHelper.GetAdHocReportSonicUTraining(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
                    ...
                    if (!string.IsNullOrEmpty(strFirstGroupBy))
                    {
                        if (strGroupByValue_1 != Convert.ToString(Reader[strFirstGroupBy]))
                        {
                            ...
                            else if (strFormatFirstGroupBy == "datetime")
                            {
                                ...
                                else sbRecord.Append("<tr><td style='font-weight: bold;'>" + strFirstGroupBy + ": " + clsGeneral.FormatDBNullDateToDisplay(strGroupByValue_1) + "</td></tr>");
                            }
                        ...
                        if (strGroupByValue_2 != Convert.ToString(Reader[strSecGroupBy]))
                        {
                            ...
                            else if (strFormatSecGroupBy == "datetime")
                            {
                                ...
                                if (strSecGroupBy == "Time Theft Reported")
                                    sbRecord.Append("<tr><td style='font-weight: bold;'>" + strSecGroupBy + ": " + string.Format("{0:HH:mm}", Reader[strSecGroupBy]) + "</td></tr>");
                                else sbRecord.Append("<tr><td style='font-weight: bold;' >" + strSecGroupBy + ": " + clsGeneral.FormatDBNullDateToDisplay(strGroupByValue_2) + "</td></tr>");
                            }
                    ...
                    if (IsAllNull == false)
                    {
                        ...
                        for (int intColumn = 0; intColumn < Reader.FieldCount; intColumn++)
                        {
                            ...
                            if (strFirstGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) && strSecGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]))
                            {
                                ...
                                {
                                    // it display only Time
                                    string strdate = string.Format("{0:MM/dd/yyyy}", Reader[intColumn]);
                                    if (strdate == "01/01/1753")
                                    {
                                        sbRecord.Append("<td></td>");
                                    }
                                    else
    ...
    {
        Sonic_U_Train_AdHocReport objAdHocReport = new Sonic_U_Train_AdHocReport();
        ...
        {
            // Delete 
            Sonic_U_Train_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<Sonic_U_Train_AdhocReportFields> lstAdhoc = new List<Sonic_U_Train_AdhocReportFields>();
    ...
    {
        Sonic_U_Train_AdHocFilter objFilter = new Sonic_U_Train_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(Sonic_U_Train_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    private void LoadFilterControlDropDown(string Field_Header, string ConditionValue, ListBox lst_F)
    {
        ...
        if (Convert.ToString(Field_Header).ToLower().Trim() == "code")
        {
            ComboHelper.FillCode(new ListBox[] { lst_F }, false);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "dealership")
        {
            ComboHelper.FillDealershipName(new ListBox[] { lst_F }, false);
        }
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "learning program")
        {
            ComboHelper.FillSonicUTrainLearningProgram(new ListBox[] { lst_F }, false);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "asset")
        {
            ComboHelper.FillSonicUTrainLearningAsset(new ListBox[] { lst_F }, false);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "learning program status")
        {
            ComboHelper.FillSonicUTrainLearningProgramStatus(new ListBox[] { lst_F }, false);
        }
        ...
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "Sonic_U_Train");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(Sonic_U_Train_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
        ...
        if (objFilter.Field_Header == "Year" || objFilter.Field_Header == "Hire Quarter" || objFilter.Field_Header == "Asset Expected Duration" || objFilter.Field_Header == "Asset Actual Duration")
        {
            isDollarSign = false;
            bShowDecPoint = false;
        }
