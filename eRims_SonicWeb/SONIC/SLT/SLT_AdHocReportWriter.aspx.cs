...
public partial class SLT_AdHocReportWriter : clsBasePage
{
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if ((blnHTML2Excel==true) && File.Exists(strFilePath))
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename=\"" + "SLT Ad-Hoc Report.xlsx" + "\""));
    ...
    protected void lnkSendEmail_Click(object sender, EventArgs e)
    {
        ...
        {
            if (clsGeneral.SendAdHocReport("Ad Hoc Report", outputFiles, "SLT Ad-Hoc Report.xlsx", Convert.ToDecimal(ddlRecipientList.SelectedItem.Value)))
            {
    ...
    {
        DataTable dtAdHocReport = SLT_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            SLT_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<SLT_AdHocFilter> lstFilter = new List<SLT_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new SLT_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            SLT_AdHocReport ObjAdHocReport = new SLT_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<SLT_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = SLT_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<SLT_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new SLT_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    ...
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "SLT Member Department".ToLower())
                    {
                        ComboHelper.Fill_DepartmentAdHoc(new ListBox[] { lst_F }, 0, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Inspection - Focus Area".ToLower())
                    {
                    ...
                    {
                        ComboHelper.FillSLT_DepartmentObserved(new ListBox[] { lst_F });
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "safety walk question - month")
                    {
                        ComboHelper.BindMonth(lst_F);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "location state")
                    {
                        ComboHelper.FillStateList(new ListBox[] { lst_F }, false);
                    }
                    else
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "SLT");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = SLT_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = SLT_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new SLT_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            Reader = AdHocReportHelper.GetAdHocReportSLT(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
            ...
            if (Reader.Read())
            {
                ...
                {
                    sbRecord.Append("<br />");
                    sbRecord.Append("<b>Report Title : SLT Ad-Hoc Report </b>");
    ...
    {
        SLT_AdHocReport objAdHocReport = new SLT_AdHocReport();
        ...
        {
            // Delete 
            SLT_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<SLT_AdhocReportFields> lstAdhoc = new List<SLT_AdhocReportFields>();
    ...
    {
        SLT_AdHocFilter objFilter = new SLT_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(SLT_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    private void LoadFilterControlDropDown(string Field_Header, string ConditionValue, ListBox lst_F)
    {
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "SLT Member Department".ToLower())
        {
            ComboHelper.Fill_DepartmentAdHoc(new ListBox[] { lst_F }, 0, false);
        }
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "safety walk question - b. departments observed")
        {
            ComboHelper.FillSLT_DepartmentObserved(new ListBox[] { lst_F });
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "safety walk question - month")
        {
            ComboHelper.BindMonth(lst_F);
        }
        else
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "SLT");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(SLT_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
