...
public partial class SONIC_Inspection_AdHocReportWriter : clsBasePage
{
    #region "ENUM"
    //Report Output Type
    ...
    /// <summary>
    /// Radio Incident Type Selected Index changed
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    //protected void rdbIncidentType_SelectedIndexChanged(object sender, EventArgs e)
    //{
    //    SetDefaults();
    //    ClearAllFilterPanel();
    //    ClearControls();
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if (File.Exists(strFilePath))
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "Inspection Ad-Hoc Report.xls"));
    ...
    {
        DataTable dtAdHocReport = Inspection_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            Inspection_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<Inspection_AdHocFilter> lstFilter = new List<Inspection_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new Inspection_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<Inspection_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = Inspection_AdHocReport.SelectReportName().Tables[0];
    ...
    //    //{
    //    lstTo.Items.Clear();
    //    lstTo.DataSource = Inspection_AdhocReportFields.GetAdHocFilterFields("O", "");
    ...
    //    //{
    //    //    lstTo.Items.Clear();
    //    //    lstTo.DataSource = Inspection_AdhocReportFields.GetAdHocFilterFields("O", rdbIncidentType.SelectedValue);
    //    //    lstTo.DataTextField = "Field_Header";
    //    //    lstTo.DataValueField = "Pk_AdhocReportFields";
    //    //    lstTo.DataBind();
    //    //}
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<Inspection_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new Inspection_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "focus area")
                    {
                        ComboHelper.Fill_FocusAreaAdHoc(new ListBox[] { lst_F }, 0, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "question text")
                    {
                    ...
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "region name")
                    {
    ...
    public string GetListBoxWhereCondition(string lstWhereFiels, string strCondition)
    {
        ...
        if (lstWhereFiels.Contains("[Inspection_Responses].Department"))
        {
            strWhere = " AND  isnull([dbo].[fn_GetDepartmentCount]([Inspection_Responses].Department,'" + strCondition + "'),0) > 0 ";
        }
        else if (lstWhereFiels.Contains("[Inspection_Responses].Deficiency_Noted"))
        {
        ...
        else if (lstWhereFiels.Contains("[Inspection_Questions].Item_Number_Focus_Area") || lstWhereFiels.Contains("[Inspection_Questions].Question_Text"))
        {
            strWhere = " And ";
    ...
    public string GetListBoxWhereConditionByText(string lstWhereFiels, string strCondition)
    {
        ...
        if (lstWhereFiels.Contains("[Inspection_Responses].Department"))
        {
            ...
            " inner join dbo.CommaSeparatedListToTable([Inspection_Responses].Department) B on A.item = B.item),0) > 0 )";
        }
        else if (lstWhereFiels.Contains("[Inspection_Responses].Deficiency_Noted"))
        {
            strWhere = " And  CAST(" + lstWhereFiels + " AS VARCHAR(7999)) IN ('" + strCondition + "') ";
        ...
        else if (lstWhereFiels.Contains("[Inspection_Questions].Item_Number_Focus_Area") || lstWhereFiels.Contains("[Inspection_Questions].Question_Text") || lstWhereFiels.Contains("[LU_Location].DBA") || lstWhereFiels.Contains("[LU_Location].Legal_Entity"))
        {
            strWhere = " And ";
            for (int intI = 0; intI < arrwhere.Length; intI++)
            {
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = Inspection_AdhocReportFields.GetAdHocFilterFields("F", "");
    ...
    {
        lstOutputFields.Items.Clear();
        lstOutputFields.DataSource = Inspection_AdhocReportFields.GetAdHocFilterFields("O", "");
        lstOutputFields.DataTextField = "Field_Header";
        lstOutputFields.DataValueField = "Pk_AdhocReportFields";
        lstOutputFields.DataBind();
        //Didable button
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new Inspection_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            //else
            // shweta - change Required
            Reader = AdHocReportHelper.GetAdHocReportInspection(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
    ...
    private void SaveReport(string strYesNo)
    {
       ...
       // objAdHocReport.IncidentType = Convert.ToString(rdbIncidentType.SelectedValue);
        ...
        {
            // Delete 
            Inspection_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<Inspection_AdhocReportFields> lstAdhoc = new List<Inspection_AdhocReportFields>();
    ...
    {
        Inspection_AdHocFilter objFilter = new Inspection_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(Inspection_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    /// </summary>
    private void LoadFilterControlAmount(Inspection_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
