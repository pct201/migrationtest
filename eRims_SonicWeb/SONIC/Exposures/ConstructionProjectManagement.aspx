                                        ...
                                        <asp:GridView ID="gvProjectList" runat="server" Width="100%" EmptyDataText="No Record Found" AllowSorting="true" OnSorting="gvProjectList_Sorting"
                                            ...
                                            <Columns>
                                                ...
                                                <asp:TemplateField HeaderText="Estimated Start Date" SortExpression="Estimated_Start_Date">
                                                    <ItemStyle Width="20%" />
                                                    <ItemTemplate>
                                                        <a href='ConstructionProjectsView.aspx?loc=<%# Encryption.Encrypt(Eval("FK_Location").ToString()) %>&prjId=<%# Encryption.Encrypt(Eval("PK_Facility_construction_Project").ToString()) %>'><%# Eval("Estimated_Start_Date") %></a>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:TemplateField HeaderText="Estimated Complete Date" SortExpression="Estimated_Completion_Date">
                                                    <ItemStyle Width="20%" />
                                                    <ItemTemplate>
                                                        <a href='ConstructionProjectsView.aspx?loc=<%# Encryption.Encrypt(Eval("FK_Location").ToString()) %>&prjId=<%# Encryption.Encrypt(Eval("PK_Facility_construction_Project").ToString()) %>'><%# Eval("Estimated_Completion_Date") %></a>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
