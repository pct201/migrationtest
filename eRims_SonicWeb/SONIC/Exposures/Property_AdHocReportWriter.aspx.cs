...
public partial class Property_AdHocReportWriter : clsBasePage
{
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if (File.Exists(strFilePath))
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "Property Ad-Hoc Report.xls"));
    ...
    protected void lnkSendEmail_Click(object sender, EventArgs e)
    {
        ...
        if (File.Exists(strFilePath))
        {
            if (clsGeneral.SendAdHocReport("Ad Hoc Report", strFilePath, "Property Ad-Hoc Report.xls", Convert.ToDecimal(ddlRecipientList.SelectedItem.Value)))
    ...
    {
        DataTable dtAdHocReport = Property_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            Property_AdHocReport.Delete(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void drpAmount_F_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (drpDwn.SelectedItem.Text == "Safety Walk - Overwrite Points" || drpDwn.SelectedItem.Text == "Incident Review - Overwrite Points" ||
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<Property_AdHocFilter> lstFilter = new List<Property_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new Property_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            Property_AdHocReport ObjAdHocReport = new Property_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<Property_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = Property_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        if (decSelectedValue > 0)
            lstAdHoc = new Property_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Location d/b/a".ToLower())
                    {
                        ComboHelper.FillLocationDBA_AllCRMAdHoc(new ListBox[] { lst_F }, 0, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "sonic location code".ToLower())
                    {
                    ...
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).Trim() == "Building Ownership")
                    {
                        ComboHelper.FillBuildingOwnerShip(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).Trim() == "Distance from body of water")
                    {
                        ComboHelper.FillDistanceFromOthers(lst_F);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).Trim() == "Fire Department Distance")
                    {
                        ComboHelper.FillDistanceFromOthers(lst_F);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).Trim() == "Fire Department Type")
                    {
                        ComboHelper.FillBuildingFireDeptTypes(lst_F);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).Trim() == "Building-Liability")
                    {
                        ComboHelper.FillBuildingLiability(lst_F);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Building State".ToLower() || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Property Cope State".ToLower()
                        || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "SecuCam State".ToLower() || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Guard State".ToLower()
                        || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Landlord State".ToLower() || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Sub-lease State".ToLower()
                        || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Intru State".ToLower() || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Fire State".ToLower()
                        || Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Additional Insureds Payees -State".ToLower())
                    {
                        DataTable dtState = State.SelectAll().Tables[0];
                        lst_F.DataSource = dtState;
                        lst_F.DataTextField = "FLD_state";
                        lst_F.DataValueField = "FLD_state";
                        lst_F.DataBind();
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Contacts-Type".ToLower())
                    {
                        ComboHelper.FillPropertyContactTypeForAdhoc(new ListBox[] { lst_F }, true);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Building Improvement-Status".ToLower())
                    {
                        ComboHelper.FillBuildingImprovementStatusForAdhoc(new ListBox[] { lst_F }, true);
                    }
                    ...
                    {
                        ComboHelper.FillBIRevisedSquareFootage(lst_F);
                    }
                   
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Fire Alarm Monitoring State".ToLower())
                    {
                        ComboHelper.FillStateList(new ListBox[] { lst_F }, false);
                    }
                    
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Location Number".ToLower())
                    {
                        ComboHelper.FillLocationLocationCodeFranchiseReport(new ListBox[] { lst_F }, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Additional Insureds Payees-Type".ToLower())
                    {
                        ComboHelper.FillLosspayeeTypes(lst_F);
                    }
                    else
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "Property");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
        ...
        foreach (ListItem lstBoxItem in lstBox.Items)
        {
            if (lstBoxItem.Selected)
            {
                if (isStringValue)
                    ...
                    strValues = strValues + "'" + lstBoxItem.Value + "'" + ",";
                    //strValues = strValues + lstBoxItem.Value.Replace("'", "''").Replace(",", "^") + ",";
    ...
    public string GetListBoxWhereCondition(string lstWhereFiels, string strCondition)
    {
        ...
        if (!string.IsNullOrEmpty(lstWhereFiels) && lstWhereFiels == "[S].Actual_Project_Completion_Date")
        {
            if (strCondition == "'0'")
            {
                strWhere = " And " + lstWhereFiels + " <= (" + DateTime.Today.ToString("dd/MM/yyyy") + ") ";
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = Property_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = Property_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new Property_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            if (drpFilter1.SelectedIndex > 0)
            {
                ...
                {
                    if (lstAdhoc[iSelected].Field_Header.ToLower() == "renewal options" || lstAdhoc[iSelected].Field_Header.ToLower() == "Contact Name".ToLower())
            ...
            Reader = AdHocReportHelper.GetAdHocReportProperty(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
            ...
            if (Reader.Read())
            {
                ...
                {
                    sbRecord.Append("<br />");
                    sbRecord.Append("<b>Report Title : Property Ad-Hoc Report </b>");
                    ...
                    if (!string.IsNullOrEmpty(strFirstGroupBy))
                    {
                        if (strGroupByValue_1 != Convert.ToString(Reader[strFirstGroupBy]))
                        {
                            ...
                            if (strFormatFirstGroupBy == "decimal")
                                sbRecord.Append("<tr><td style='font-weight: bold;color: #FF9C09;' >&nbsp;" + strFirstGroupBy + ": " + string.Format("{0:c2}", strGroupByValue_1) + "</td></tr>");
                    ...
                    if (!string.IsNullOrEmpty(strSecGroupBy))
                    {
                        if (strGroupByValue_2 != Convert.ToString(Reader[strSecGroupBy]))
                        {
                            ...
                            if (strFormatSecGroupBy == "decimal")
                                sbRecord.Append("<tr><td style='font-weight: bold; bold;color: #276692;' >&nbsp;" + strSecGroupBy + ": " + string.Format("{0:c2}", strGroupByValue_2) + "</td></tr>");
                            else if (strFormatSecGroupBy == "datetime")
                            {
                                sbRecord.Append("<tr><td style='font-weight: bold; bold;color: #276692;' >&nbsp;" + strSecGroupBy + ": " + clsGeneral.FormatDBNullDateToDisplay(strGroupByValue_2) + "</td></tr>");
                            }
                            else sbRecord.Append("<tr><td style='font-weight: bold; bold;color: #276692;' >&nbsp;" + strSecGroupBy + ": " + strGroupByValue_2 + "</td></tr>");
                        }
                        else if (Reader[strSecGroupBy] == DBNull.Value && strNOGroup2 == string.Empty)
                        {
                            strNOGroup2 = "No " + strSecGroupBy;
                            sbRecord.Append("<tr><td style='font-weight: bold; bold;color: #276692;' >&nbsp;" + strSecGroupBy + ": " + strNOGroup2 + "</td></tr>");
                            //No Group by assign
                    ...
                    if (IsAllNull == false)
                    {
                        ...
                        for (int intColumn = 0; intColumn < Reader.FieldCount; intColumn++)
                        {
                            ...
                            if (strFirstGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) && strSecGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]))
                            {
                                ...
                                {
                                    if (Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) == "Associate Employee Number")
    ...
    private string GenerateFilterCriteria()
    {
        ...
        strExport.Append("<tr>");
        strExport.Append("<td><b>Report Title :Property Ad-Hoc Report</b></td>");
        strExport.Append("</tr>");
        strExport.Append("<tr><td>&nbsp;</td></tr>");
        if (drpGroupByFirst.SelectedIndex > 0)
        ...
        strExport.Append("<tr><td ></td></tr>");
        if (drpSortingFirst.SelectedIndex > 0)
            strExport.Append("<tr><td nowrap='nowrap'><b> First Level Sort By : </b>" + Convert.ToString(drpSortingFirst.SelectedItem.Text) + " <b>Sorting :</b> " + Convert.ToString(rdbSort1.SelectedItem.Text).ToUpper() + " </td></tr>");
        else
            strExport.Append("<tr><td ><b> First Level Sort By : </b> &nbsp; </td></tr>");

        if (drpSortingSecond.SelectedIndex > 0)
            strExport.Append("<tr><td nowrap='nowrap'><b> Second Level Sort By : </b>" + Convert.ToString(drpSortingSecond.SelectedItem.Text) + " <b>Sorting :</b> " + Convert.ToString(rdbSort2.SelectedItem.Text).ToUpper() + " </td></tr>");
        else
            strExport.Append("<tr><td ><b> Second Level Sort By : </b> &nbsp; </td></tr>");

        if (drpSortingThird.SelectedIndex > 0)
            strExport.Append("<tr><td nowrap='nowrap'><b> Third Level Sort By : </b>" + Convert.ToString(drpSortingThird.SelectedItem.Text) + " <b>Sorting :</b> " + Convert.ToString(rdbSort3.SelectedItem.Text).ToUpper() + " </td></tr>");
        else
            strExport.Append("<tr><td ><b> Third Level Sort By : </b> &nbsp; </td></tr>");
    ...
    private string GetClaimSummaryFiltersDetails()
    {
        ...
        strWhere.Append(GetSelectedFilters(drpFilter1, drpText_F1, txtFilter1, lst_F1, lstDate1, txtDate_From1, txtDate_To1, drpAmount_F1, txtAmount1_F1, txtAmount2_F1));
        strWhere.Append(GetSelectedFilters(drpFilter2, drpText_F2, txtFilter2, lst_F2, lstDate2, txtDate_From2, txtDateTo2, drpAmount_F2, txtAmount1_F2, txtAmount2_F2));
        strWhere.Append(GetSelectedFilters(drpFilter3, drpText_F3, txtFilter3, lst_F3, lstDate3, txtDate_From3, txtDate_To3, drpAmount_F3, txtAmount1_F3, txtAmount2_F3));
        strWhere.Append(GetSelectedFilters(drpFilter4, drpText_F4, txtFilter4, lst_F4, lstDate4, txtDate_From4, txtDate_To4, drpAmount_F4, txtAmount1_F4, txtAmount2_F4));
        strWhere.Append(GetSelectedFilters(drpFilter5, drpText_F5, txtFilter5, lst_F5, lstDate5, txtDate_From5, txtDate_To5, drpAmount_F5, txtAmount1_F5, txtAmount2_F5));
        strWhere.Append(GetSelectedFilters(drpFilter6, drpText_F6, txtFilter6, lst_F6, lstDate6, txtDate_From6, txtDate_To6, drpAmount_F6, txtAmount1_F6, txtAmount2_F6));
        strWhere.Append(GetSelectedFilters(drpFilter7, drpText_F7, txtFilter7, lst_F7, lstDate7, txtDate_From7, txtDate_To7, drpAmount_F7, txtAmount1_F7, txtAmount2_F7));
        strWhere.Append(GetSelectedFilters(drpFilter8, drpText_F8, txtFilter8, lst_F8, lstDate8, txtDate_From8, txtDate_To8, drpAmount_F8, txtAmount1_F8, txtAmount2_F8));
        strWhere.Append(GetSelectedFilters(drpFilter9, drpText_F9, txtFilter9, lst_F9, lstDate9, txtDate_From9, txtDate_To9, drpAmount_F9, txtAmount1_F9, txtAmount2_F9));
        strWhere.Append(GetSelectedFilters(drpFilter10, drpText_F10, txtFilter10, lst_F10, lstDate10, txtDate_From10, txtDate_To10, drpAmount_F10, txtAmount1_F10, txtAmount2_F10));

        return strWhere.ToString();
    }

    public string GetSelectedFilters(DropDownList drpFilter, DropDownList drpText_F, TextBox txtFilter, ListBox lst_F, DropDownList lstDate, TextBox txtDate_From, TextBox txtDate_To, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F)
    {
        List<Property_AdhocReportFields> lstAdhoc = null;
        decimal decValue;
        int iSelected;
        ...
        if (drpFilter.SelectedIndex > 0)
        {
            ...
            strWhere = "<tr><td colspan=6>" + strWhere + "</td></tr>";
        }
        return strWhere;
    ...
    {
        Property_AdHocReport objAdHocReport = new Property_AdHocReport();
        ...
        {
            // Delete 
            Property_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<Property_AdhocReportFields> lstAdhoc = new List<Property_AdhocReportFields>();
    ...
    {
        Property_AdHocFilter objFilter = new Property_AdHocFilter();
        ...
        else if (Fk_ControlType == (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList)
        {
            objFilter.ConditionValue = ListFilterWhere;
        }
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(Property_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    {
        lst_F.Items.Clear();
        if (Convert.ToString(Field_Header).ToLower().Trim() == "Location d/b/a".ToLower())
        {
            ComboHelper.FillLocationDBA_AllCRMAdHoc(new ListBox[] { lst_F }, 0, false);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Inspection - Focus Area".ToLower())
        {
            ComboHelper.Fill_FocusAreaAdHoc(new ListBox[] { lst_F }, 0, false);
        }
       
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "sonic location code".ToLower())
        {
            ComboHelper.FillLocationLocationCodeFranchiseReport(new ListBox[] { lst_F }, false);
        }
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "distance from body of water")
        {
            ComboHelper.FillDistanceFromOthers(lst_F);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "fire department distance")
        {
            ComboHelper.FillDistanceFromOthers(lst_F);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "fire department type")
        {
            ComboHelper.FillBuildingFireDeptTypes(lst_F);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "building-liability")
        {
            ComboHelper.FillBuildingLiability(lst_F);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Building State".ToLower() || Convert.ToString(Field_Header).ToLower().Trim() == "Property Cope State".ToLower()
                        || Convert.ToString(Field_Header).ToLower().Trim() == "SecuCam State".ToLower() || Convert.ToString(Field_Header).ToLower().Trim() == "Guard State".ToLower()
                        || Convert.ToString(Field_Header).ToLower().Trim() == "Landlord State".ToLower() || Convert.ToString(Field_Header).ToLower().Trim() == "Sub-lease State".ToLower()
                        || Convert.ToString(Field_Header).ToLower().Trim() == "Intru State".ToLower() || Convert.ToString(Field_Header).ToLower().Trim() == "Fire State".ToLower()
                        || Convert.ToString(Field_Header).ToLower().Trim() == "Additional Insureds Payees -State".ToLower())
        {
            DataTable dtState = State.SelectAll().Tables[0];
            lst_F.DataSource = dtState;
            lst_F.DataTextField = "FLD_state";
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Building Improvement-Status".ToLower())
        {
            ComboHelper.FillBuildingImprovementStatusForAdhoc(new ListBox[] { lst_F }, true);
        }
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Contacts-Type".ToLower())
        {
            ComboHelper.FillPropertyContactTypeForAdhoc(new ListBox[] { lst_F }, true);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Fire Alarm Monitoring State".ToLower())
        {
            ComboHelper.FillStateList(new ListBox[] { lst_F }, false);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Building Improvement-Revised Square Footage".ToLower())
        {
            ComboHelper.FillBIRevisedSquareFootage(lst_F);
        }
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "Additional Insureds Payees-Type".ToLower())
        {
            ComboHelper.FillLosspayeeTypes(lst_F);
        }
        else
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "Property");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(Property_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
        ...
        if (objFilter.Field_Header == "Safety Walk - Overwrite Points" || objFilter.Field_Header == "Incident Review - Overwrite Points" ||
    ...
    private void SetSelectedValueFromString(string CSValue, ListBox lstSelect)
    {
        if (!string.IsNullOrEmpty(CSValue))
        {
            ...
            {
                li = lstSelect.Items.FindByValue(strSelect.TrimStart('\'').TrimEnd('\''));
