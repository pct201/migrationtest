...
public partial class EPM_AdHocReportWriter : clsBasePage
{
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        if (File.Exists(strFilePath))
        {
            try
            {
                ...
                HttpContext.Current.Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "EPM Ad-Hoc Report.xls"));
    ...
    {
        DataTable dtAdHocReport = EPM_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            EPM_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            // Set Report PK
            _dcSelectedReport = 0;
            List<EPM_AdHocFilter> lstFilter = new List<EPM_AdHocFilter>();

            _dcSelectedReport = Convert.ToDecimal(ddlReports.SelectedItem.Value);
            lstFilter = new EPM_AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            EPM_AdHocReport ObjAdHocReport = new EPM_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<EPM_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = EPM_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<EPM_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new EPM_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Sonic Location d/b/a".ToLower())
                    {
                        ComboHelper.FillLocationDBA_AllCRMAdHoc(new ListBox[] { lst_F }, 0, false);
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Consultant".ToLower())
                    {
                        lst_F.Items.Clear();
                        lst_F.Items.Add(new ListItem("Hart & Hickman", "1"));
                        lst_F.Items.Add(new ListItem("Other Consultant", "0"));
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Environment Project Management Status".ToLower())
                    {
                        lst_F.Items.Clear();
                        lst_F.Items.Add(new ListItem("Open", "1"));
                        lst_F.Items.Add(new ListItem("Closed", "0"));
                    }
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "E-Mail Recipients".ToLower())
                    {
                    ...
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "Building State".ToLower())
                    {
                    ...
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "EPM");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
    ...
    public string GetDateWhereCondtion(string strFieldName, string strDatefrom, string strDateTo, string cblDateCriteria)
    {
        ...
        if (dtFrom.HasValue)
            strWhere = AdHocReportHelper.GetDateWhereAbsolute_EPMAdhoc(strFieldName, dtFrom, dtTo, DtType);
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = EPM_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = EPM_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new EPM_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            Reader = AdHocReportHelper.GetAdHocReportEPM(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
            ...
            if (Reader.Read())
            {
                ...
                {
                    sbRecord.Append("<br />");
                    sbRecord.Append("<b>Report Title : EPM Ad-Hoc Report </b>");
                ...
                foreach (DataRow drHeader in dtSchema.Rows)
                {
                    ...
                    {
                        if (Convert.ToString(drHeader["ColumnName"]) == "Action And Notes Status")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Project Description")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Comments/Description")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Comments")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Isssues")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Action")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else if (Convert.ToString(drHeader["ColumnName"]) == "Required Activity Description")
                            sbRecord.Append("<td width='705'><b>" + drHeader["ColumnName"] + "</b></td>");
                        else
                    ...
                    if (IsAllNull == false)
                    {
                        ...
                        for (int intColumn = 0; intColumn < Reader.FieldCount; intColumn++)
                        {
                            ...
                            if (strFirstGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) && strSecGroupBy != Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]))
                            {
                                ...
                                {
                                    if (Convert.ToString(dtSchema.Rows[intColumn]["ColumnName"]) == "Investigations - First Report Number")
                                    {
                                        //sbRecord.Append("<td align='right'>" + string.Format("{0:c2}", Reader[intColumn]) + "</td>");
                                        sbRecord.Append("<td align='right' valign='top'>" + Reader[intColumn].ToString() + "</td>");
                                    }
                                    else
                                        sbRecord.Append("<td align='right' valign='top'>" + string.Format("{0:c2}", Reader[intColumn]) + "</td>");
                                ...
                                else if (strFormat == "datetime")
                                {
                                    ...
                                    {
                                        sbRecord.Append("<td valign='top'>" + string.Format("{0:MM/dd/yyyy}", Reader[intColumn]) + "</td>");
                                    }
                                }
                                else
                                {
                                    ...
                                    sbRecord.Append(@"<td valign='top'>" + Convert.ToString(Reader[intColumn]) + "</td>");
                                }
                            }
    ...
    {
        EPM_AdHocReport objAdHocReport = new EPM_AdHocReport();
        ...
        {
            // Delete 
            EPM_AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<EPM_AdhocReportFields> lstAdhoc = new List<EPM_AdhocReportFields>();
    ...
    {
        EPM_AdHocFilter objFilter = new EPM_AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(EPM_AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    {
        lst_F.Items.Clear();
        if (Convert.ToString(Field_Header).ToLower().Trim() == "Sonic Location d/b/a".ToLower())
        {
            ComboHelper.FillLocationDBA_AllCRMAdHoc(new ListBox[] { lst_F }, 0, false);
        }
        ...
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "EPM");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(EPM_AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
