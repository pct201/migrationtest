...
public partial class SONIC_Exposures_AuditPopup_Project_Management : clsBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["id"]))
            {
                ...
                // show Case Grid 
                dvGrid.Visible = true;
                lbltable_Name.Text = "Project Management";

                // Fill grid with audit table
                dt = AuditTrail.GetProjectManagement_AuditTrial(Convert.ToDecimal(Request.QueryString["id"])).Tables[0];
                gvProjectManagement.DataSource = dt;
