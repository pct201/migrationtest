...
public partial class SONIC_Exposures_EPMActionResultMail : System.Web.UI.Page
{
            ...
            if (EmailType == 0)
            {
                ...
                if (TableName == "APNOTES")
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["PK_Fields"]) && !string.IsNullOrEmpty(Request.QueryString["Table_Name"]) && !string.IsNullOrEmpty(Request.QueryString["Claim_ID"]))
                    {
                        ...
                        dtNotes = null;
                        dtNotes = clsEPM_Action_Notes_RM.SelectByIDs(PK_Fields).Tables[0];
                        ...
                        string strDBA = "";
                        string strNotes = "Selected EPM Action Notes";
                        string strDocName = "EPMActionNotes.doc";
                        string strAddress = "";
                        string strCity = "";
                        string strZip = "";
                        string strState = "";
                        ...
                        foreach (DataRow drClaims_Adjustor_Notes in dtNotes.Rows)
                        {
                            ...
                            {
                                sbHTML.Append("<td align='left' valign='top'>" + clsGeneral.FormatDBNullDateToDisplay(clsGeneral.FormatDBNullDateToDate(Convert.ToDateTime(drClaims_Adjustor_Notes["Note_Date"].ToString()))) + "</td>");
                            }
                            else
                            {
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
        if (EmailType == 0)
        {
            clsGeneral.SendMailMessage(AppConfig.MailFrom, txtTo.Text.Trim(), string.Empty, string.Empty, "Selected EPM Action Notes", txtBody.Text.Trim().Replace("\r\n", "<br/>"), true, arrAttachments);
        }
        else
