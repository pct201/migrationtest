...
public partial class SONIC_Exposures_AuditPopup_BuildingOwnership_Sublease : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                // show Case Grid 
                dvGrid.Visible = true;
                lbltable_Name.Text = "Building_Ownership_Sublease";

                // Fill grid with audit table
                dt = AuditTrail.GetBuilding_Ownership_Sublease_AuditTrail(Convert.ToDecimal(Request.QueryString["id"])).Tables[0];
                gvBuildingOwnership_Sublease.DataSource = dt;
                gvBuildingOwnership_Sublease.DataBind();
