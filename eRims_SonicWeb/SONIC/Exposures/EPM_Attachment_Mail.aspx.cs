...
public partial class SONIC_Exposures_EPM_Attachment_Mail : clsBasePage
{
    #region " Properties "
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                DataTable dtAttachments = clsEPM_Attachments.GetAttchmentsByID(Request.QueryString["Attch_Ids"].ToString()).Tables[0];
                m_arrGlobalPath = new string[Request.QueryString["Attch_Ids"].ToString().Split(',').Length];
                m_strGlobalPath = AppConfig.strEPMDocumentPath;
