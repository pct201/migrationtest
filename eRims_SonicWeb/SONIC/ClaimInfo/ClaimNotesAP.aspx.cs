...
public partial class ClaimNotesAP : clsBasePage
{
    #region Properties
    ...
    public decimal PK_AP_Notes
    {
        get
        {
            return clsGeneral.GetLong(ViewState["PK_AP_Notes"]);
        }
        set { ViewState["PK_AP_Notes"] = value; }
    }
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (CheckClaimInfo())
            {
                if (!string.IsNullOrEmpty(Request.QueryString["viewIDs"]))
                {
                    ...
                    // Rebind Grid
                    rptNotes.DataSource = clsAP_Notes.SelectByIDs(Encryption.Decrypt(Request.QueryString["viewIDs"]));
                    rptNotes.DataBind();
    ...
    {
        clsAP_Notes objAP_Notes = new clsAP_Notes(PK_AP_Notes);

        // set Control Value to Objects
        if (!string.IsNullOrEmpty(txtNote_Date.Text))
            objAP_Notes.Note_Date = Convert.ToDateTime(txtNote_Date.Text.Trim());
        objAP_Notes.Note = txtNotes.Text.Trim();
        objAP_Notes.FK_Table = FK_Claim;
        objAP_Notes.FK_Table_Name = FK_Table_Name;
        objAP_Notes.Update_Date = DateTime.Now;
        objAP_Notes.Updated_By = clsSession.UserID;

        // Check if Pk Is Greater Than 0 then Update Record else Add Record.
        if (PK_AP_Notes > 0)
        {
            // Update Records
            objAP_Notes.PK_AP_Notes = PK_AP_Notes;
            objAP_Notes.Update();
        }
        else
        {
            // Insert New Record
            PK_AP_Notes = objAP_Notes.Insert();
        }
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["loc"]) && !string.IsNullOrEmpty(Request.QueryString["page"]) && !string.IsNullOrEmpty(Request.QueryString["op"]))
        {
            ...
            Response.Redirect(Request.AppRelativeCurrentExecutionFilePath + "?id=" + Encryption.Encrypt(PK_AP_Notes.ToString()) + "&FK_Claim=" + Encryption.Encrypt(FK_Claim.ToString()) + "&op=view" + "&page=" + PageToRediect + "&loc=" + Encryption.Encrypt(Location_ID.ToString()));
        }
        else
        {
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["loc"]) && !string.IsNullOrEmpty(Request.QueryString["page"]) && !string.IsNullOrEmpty(Request.QueryString["op"]))
        {
            ...
            Response.Redirect(Request.AppRelativeCurrentExecutionFilePath + "?id=" + Encryption.Encrypt(PK_AP_Notes.ToString()) + "&FK_Claim=" + Encryption.Encrypt(FK_Claim.ToString()) + "&op=edit" + "&page=" + PageToRediect + "&loc=" + Encryption.Encrypt(Location_ID.ToString()));
        }
        else
        {
    ...
    private void BindControlForEdit()
    {
        ...
        // Bind Control value
        txtNote_Date.Text = clsGeneral.FormatDBNullDateToDisplay(objAP_Notes.Note_Date);
        FK_Table_Name = objAP_Notes.FK_Table_Name;
        if (objAP_Notes.Note != null)
            txtNotes.Text = objAP_Notes.Note.Trim();
    ...
    private void BindControlForView()
    {
        ...
        FK_Table_Name = objAP_Notes.FK_Table_Name;
        // Bind Control value
        lblNote_Date.Text = clsGeneral.FormatDBNullDateToDisplay(objAP_Notes.Note_Date);

        if (objAP_Notes.Note != null)
            lblNotes.Text = objAP_Notes.Note.Trim();
