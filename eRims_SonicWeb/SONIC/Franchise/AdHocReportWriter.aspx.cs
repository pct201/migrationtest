...
public partial class SONIC_Franchise_AdHocReportWriter : clsBasePage
{
    ...
    {
        DataTable dtAdHocReport = Franchise_AdHocReport.ExistsReportName(txtReportName.Text, (ddlReports.SelectedIndex > 0) ? Convert.ToDecimal(ddlReports.SelectedValue) : 0);
    ...
    protected void btnDeleteReport_Click(object sender, EventArgs e)
    {
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            Franchise_AdHocReport.DeleteByPK(Convert.ToDecimal(ddlReports.SelectedItem.Value));
            hdnReportId.Value = "0";
            txtReportName.Text = "";
            SetDefaults();
            ClearAllFilterPanel();
    ...
    protected void ddlReports_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlReports.SelectedIndex > 0)
        {
            ...
            lstFilter = new AdHocFilter().GetAdHocReportFieldByPk(_dcSelectedReport.Value);

            Franchise_AdHocReport ObjAdHocReport = new Franchise_AdHocReport(_dcSelectedReport.Value);
    ...
    /// <returns></returns>
    private int SearchList(decimal decValue, List<Franchise_AdhocReportFields> lstSearch)
    {
        int i;
    ...
    private void BindReportNameDropDown()
    {
        ...
        DataTable dtReprot = Franchise_AdHocReport.SelectReportName().Tables[0];
    ...
    protected string drpFilterChange(DropDownList drpFilter, Panel pnlText_F, Panel pnlAmoun_F, Panel pnlDate_F, ListBox lst_F)
    {
        ...
        List<Franchise_AdhocReportFields> lstAdHoc = null;

        if (decSelectedValue > 0)
            lstAdHoc = new Franchise_AdhocReportFields().GetAdHocReportFieldByPk(decSelectedValue);
        ...
        if (lstAdHoc != null && lstAdHoc.Count > 0)
        {
            switch (lstAdHoc[0].Fk_ControlType.Value)
            {
                ...
                case (int)AdHocReportHelper.AdHocControlType.MultiSelectTextList:
                    ...
                    else if (Convert.ToString(lstAdHoc[0].Field_Header).ToLower().Trim() == "franchise brand image")
                    {
                        ComboHelper.Fill_LU_Franchise(new ListBox[] { lst_F }, false, ComboHelper.LU_Franchise.LU_Franchise_Additional_Brand_Image);
                    }
                    else
                    {
                        AdHocReportHelper.FillFilterDropDown(lstAdHoc[0].Field_Header, new ListBox[] { lst_F }, false, "Franchise");
                    }
                    pnlText_F.Visible = false;
                    pnlAmoun_F.Visible = false;
                    pnlDate_F.Visible = false;
    ...
    private void BindFilterCombo()
    {
        for (int intI = 1; intI <= 10; intI++)
        {
            ...
            if (drpFilter != null)
            {
                ...
                drpFilter.DataSource = Franchise_AdhocReportFields.GetAdHocFilterFields("F");
    ...
    private void BindOutpuFields()
    {
        ...
        lstOutputFields.DataSource = Franchise_AdhocReportFields.GetAdHocFilterFields("O");
    ...
    private string BindReport(ref StringBuilder sbRecord, ReportOutputType ReportType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(strCriteria))
                lstAdhoc = new Franchise_AdhocReportFields().GetAdHocReportFieldByMultipleID(strCriteria);
            ...
            if (drpFilter1.SelectedIndex > 0)
            {
                ...
                {
                    if (lstAdhoc[iSelected].Field_Header.ToLower() == "renewal options")
            ...
            Reader = AdHocReportHelper.GetAdHocReportFranchise(GetAllItemString(lstSelectedFields, false), strGroupBy, strWhere, strOrderBy, strFilterIds, Convert.ToDecimal(clsSession.UserID));
            ...
            if (Reader.Read())
            {
                ...
                {
                    sbRecord.Append("<br />");
                    sbRecord.Append("<b>Report Title : Franchise Ad-Hoc Report </b>");
    ...
    {
        Franchise_AdHocReport objAdHocReport = new Franchise_AdHocReport();
        ...
        {
            // Delete 
            AdHocFilter.DeleteByFk_AdHocReport(Pk_AdHocReport);

            string strCriteria = string.Empty;
            strCriteria = GetFilterIDs(new DropDownList[] { drpFilter1, drpFilter2, drpFilter3, drpFilter4, drpFilter5, drpFilter6, drpFilter7, drpFilter8, drpFilter9, drpFilter10, });
            List<Franchise_AdhocReportFields> lstAdhoc = new List<Franchise_AdhocReportFields>();
    ...
    {
        AdHocFilter objFilter = new AdHocFilter();
    ...
    /// <param name="drpFilter"></param>
    private void LoadFilterCriteria(AdHocFilter objFilter, DropDownList drpFilter)
    {
        ListItem liSelected;
    ...
    private void LoadFilterControlDropDown(string Field_Header, string ConditionValue, ListBox lst_F)
    {
        ...
        else if (Convert.ToString(Field_Header).ToLower().Trim() == "franchise brand image")
        {
            ComboHelper.Fill_LU_Franchise(new ListBox[] { lst_F }, false, ComboHelper.LU_Franchise.LU_Franchise_Additional_Brand_Image);
        }
        else
        {
            AdHocReportHelper.FillFilterDropDown(Field_Header, new ListBox[] { lst_F }, false, "Franchise");
        }
    ...
    /// </summary>
    private void LoadFilterControlAmount(AdHocFilter objFilter, Panel pnlAmount_F, DropDownList drpAmount_F, TextBox txtAmount1_F, TextBox txtAmount2_F, Label lblAmountText1_F, Label lblAmountText2_F, CompareValidator cvAmount)
    {
