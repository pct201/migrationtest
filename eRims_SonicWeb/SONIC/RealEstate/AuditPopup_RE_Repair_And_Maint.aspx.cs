...
public partial class SONIC_RealEstate_AuditPopup_RE_Repair_And_Maint : clsBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                // show Case Grid 
                dvGrid.Visible = true;
                lbltable_Name.Text = "RE_Repair_Maintenance";

                // Fill grid with audit table
                dt = AuditTrail.GetRE_Repair_And_Maintenance_AuditTrail(Convert.ToDecimal(Request.QueryString["id"])).Tables[0];
                gvRepair_Maintenance.DataSource = dt;
                gvRepair_Maintenance.DataBind();
