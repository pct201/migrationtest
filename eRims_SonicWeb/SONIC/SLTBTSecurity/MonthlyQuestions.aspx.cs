...
public partial class SONIC_SLT_BT_Security_Walk_MonthlyQuestions : clsBasePage
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (dtMonthlyQuestions.Rows.Count > 0)
            Response.Redirect("SLT_BTSecurity_Walk_Management_Monthly_Grid.aspx?Year=" + Encryption.Encrypt(Convert.ToString(Tmp_Year)) + "&Month=" + Encryption.Encrypt(Convert.ToString(Tmp_Month)), true);
        else
            Response.Redirect("SLT_BTSecurity_Walk_Management_Monthly.aspx?id=" + Encryption.Encrypt("0") + "&Year=" + Encryption.Encrypt(Convert.ToString(Tmp_Year)) + "&Month=" + Encryption.Encrypt(Convert.ToString(Tmp_Month)), true);
    }
     #endregion
}