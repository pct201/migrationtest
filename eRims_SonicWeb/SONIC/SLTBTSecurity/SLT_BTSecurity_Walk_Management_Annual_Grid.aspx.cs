...
public partial class SONIC_SLT_BTSecurity_Walk_Management_Annual_Grid : clsBasePage
{
    #region Properties
    /// <summary>
    ...
    {
        DataSet dsMonthlyQuestions = clsLU_SLT_BT_Security_Walk_Focus_Area.Search(Year, "", true, SortBy, SortOrder, PageNumber, PageSize);
    ...
    protected void gvAnnualQuestions_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                DataSet dsMonthlyQuestions = clsLU_SLT_BT_Security_Walk_Focus_Area.Search(Year, "", true, SortBy, SortOrder, ctrlPageAnnualQue.CurrentPage, ctrlPageAnnualQue.PageSize);
    ...
    protected void gvAnnualQuestions_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("SLT_BTSecurity_Walk_Management_Annual.aspx?id=" + Encryption.Encrypt(Convert.ToString(e.CommandArgument)) + "&Year=" + Encryption.Encrypt(Convert.ToString(Year)), true);
        }
        if (e.CommandName == "up")
        {
            clsLU_SLT_BT_Security_Walk_Focus_Area.UpdateSortOrder_Annual(Convert.ToDecimal(e.CommandArgument), e.CommandName);
            BindGrid(ctrlPageAnnualQue.CurrentPage, ctrlPageAnnualQue.PageSize);
        }
    ...
    {
        Response.Redirect("SLT_BTSecurity_Walk_Management_Annual.aspx?id=" + Encryption.Encrypt("0") + "&Year=" + Encryption.Encrypt(Convert.ToString(Year)), true);
    }
