...
public partial class Controls_Attachment_ProjectManagement_Attachment : System.Web.UI.UserControl
{
    #region " Properties "
    ...
    {
        ComboHelper.FillLU_EPM_Attachment_Type(new DropDownList[] { drpFolder,Attachment1.DropdownFolder, Attachment2.DropdownFolder, Attachment3.DropdownFolder, Attachment4.DropdownFolder, 
    ...
    protected void gvFiles_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            lnkDocName.OnClientClick = "javascript:openWindow('../../Download.aspx?fname=" + Encryption.Encrypt(strFileName) + "&SLT=EPM_Docs');return false;";
        }
    }
    ...
    protected void gvFiles_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            // get PK
            PK_EPM_Attchments = Convert.ToDecimal(e.CommandArgument);

            // bind the controls in edit mode
            // also set the controls required in edit mode
            clsEPM_Attachments objEPM_Attachment = new clsEPM_Attachments(PK_EPM_Attchments);
            tblEditAttachment.Visible = true;
            drpFolder.ClearSelection();
            drpFolder.SelectedValue = objEPM_Attachment.Attachment_Type.ToString();
        ...
        if (e.CommandName == "DeleteAttachment")    // if command is for delete
        {
            ...
            clsEPM_Attachments.DeleteByPK(Convert.ToDecimal(strArgs[0]));
    ...
    {
        DataTable dtFolder = clsEPM_Attachments.GetAttchmentFolderAndCount(FK_EPM_Identification).Tables[0];
    ...
    private void BindGridFiles()
    {
        ...
        if (FK_EPM_Identification > 0)
        {
            ...
            DataTable dtEPM_Attachment = clsEPM_Attachments.SelectFilesByAttachmentType(strFK_IDs, FK_EPM_Identification).Tables[0];
            ...
            // bind the grid
            dtEPM_Attachment.DefaultView.Sort = _SortBy + " " + _SortOrder;
            gvFiles.DataSource = dtEPM_Attachment;
            gvFiles.DataBind();
            btnViewPDF.Visible = false; //(dtAttachment.Rows.Count > 0);
            btnEmail.Visible = (dtEPM_Attachment.Rows.Count > 0);
        ...
        {
            {
                ...
                clsEPM_Attachments objEPMAttachment;

                if (PK_EPM_Attchments > 0)
                    objEPMAttachment = new clsEPM_Attachments(PK_EPM_Attchments);
                else
                    objEPMAttachment = new clsEPM_Attachments();
                ...
                if (!clsGeneral.IsNull(fpFile.PostedFile))
                    ...
                    strExtension = objEPMAttachment.Attachment_Filename.Substring(objEPMAttachment.Attachment_Filename.LastIndexOf("."));
                ...
                else
                {
                    ...
                    objEPMAttachment.FK_EPM_Identification = FK_EPM_Identification;
                    objEPMAttachment.Attachment_Type = Convert.ToDecimal(drpFolder.SelectedValue);
                    ...
                    if (PK_EPM_Attchments > 0)
                    {
                        objEPMAttachment.Attachment_Description = objEPMAttachment.Attachment_Filename.Substring(12);
                        objEPMAttachment.Update();
                    }
                    else
                    {
                        if (!clsGeneral.IsNull(fpFile.PostedFile.FileName))
                        {
                            objEPMAttachment.Attachment_Description = fpFile.PostedFile.FileName.Substring(fpFile.PostedFile.FileName.LastIndexOf("\\") + 1);
                            // upload the document
                            string strUploadPath = AppConfig.strEPMDocumentPath;
                            // upload and set the filename.
                            objEPMAttachment.Attachment_Filename = clsGeneral.UploadFile(fpFile, strUploadPath, false, false);
                            //Insert the attachment record
                            PK_EPM_Attchments = objEPMAttachment.Insert();
                        }
                    }
    ...
    private void SaveAttachment(controls_attachment_projectmanagement_epm_attach_section_ascx obj)
    {
        if (obj.FileBrowser.HasFile && (obj.DropdownFolder.SelectedIndex > 0))
        {
            ...
            ////set values to store in database
            clsEPM_Attachments objEPM_Attachments = new clsEPM_Attachments();
            bool bValid = true;
            string strExtension;
            // check file Extensions for new file name provided by user
            ...
            if (!string.IsNullOrEmpty(obj.FileBrowser.PostedFile.FileName))
                ...
                strExtension = objEPM_Attachments.Attachment_Filename.Substring(objEPM_Attachments.Attachment_Filename.LastIndexOf("."));
            ...
            {
                // set properties from page controls
                objEPM_Attachments.FK_EPM_Identification = FK_EPM_Identification;
                objEPM_Attachments.Attachment_Type = Convert.ToDecimal(obj.DropdownFolder.SelectedValue);
                objEPM_Attachments.Updated_By = clsSession.UserID;
                objEPM_Attachments.Update_Date = DateTime.Now;
                objEPM_Attachments.Attach_Date = DateTime.Now;

                if (!string.IsNullOrEmpty(obj.FileBrowser.PostedFile.FileName))
                {
                    objEPM_Attachments.Attachment_Description = obj.FileBrowser.PostedFile.FileName.Substring(obj.FileBrowser.PostedFile.FileName.LastIndexOf("\\") + 1);
                    // upload the document
                    string strUploadPath = AppConfig.strEPMDocumentPath;
                    // upload and set the filename.
                    objEPM_Attachments.Attachment_Filename = clsGeneral.UploadFile(obj.FileBrowser, strUploadPath, false, false);

                    //Insert the attachment record
                    PK_EPM_Attchments = objEPM_Attachments.Insert();
    ...
    private void SetDefaultsForAttachment(controls_attachment_projectmanagement_epm_attach_section_ascx objAttachemnt)
    {
