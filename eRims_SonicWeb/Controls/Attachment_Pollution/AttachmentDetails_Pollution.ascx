<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AttachmentDetails_Pollution.ascx.cs"
    Inherits="AttachmentDetails_Pollution" %>
...
<script type="text/javascript">
    ...
    function ShowMailPage(attTbl, PK_Field_Name) {
        ...
        if (cnt == 0) {
            ...
            var oWnd = window.open('<%=AppConfig.SiteURL%>SONIC/Pollution/SendEmail.aspx?EmailType= <%=Convert.ToString((int)clsGeneral.EmailTYpe.Attachment)%>&AttachmentTable=' + attTbl + '&PK_Field_Name=' + PK_Field_Name + '&AttachmentId=' + m_strAttIds, "Erims", "location=0,status=0,scrollbars=1,menubar=0,resizable=1,toolbar=0,width=600,height=300");
            oWnd.moveTo(260, 180);
            return false;
        }
    }
            ...
            <asp:GridView ID="gvAttachment" runat="server" AllowPaging="false" AutoGenerateColumns="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Attachment Type">
                        ...
                        <ItemTemplate>
                            <asp:Label ID="lblAttachment_Type" runat="server" Width="250px" Text='<%#Eval("Attachment_Type")%>'
                                CssClass="textAttachment"></asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
