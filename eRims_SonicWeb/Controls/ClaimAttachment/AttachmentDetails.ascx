<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AttachmentDetails.ascx.cs"
    Inherits="Controls_ClaimAttachment_AttachmentDetails" %>
...
<script type="text/javascript">
    function CheckSelected1()
    {
        ...
        {
            if(ctrls[i].type=="checkbox" && ctrls[i].id.indexOf("chkSelectClaim") > 0)
            {
                if(ctrls[i].checked)
                    cnt++;
            }
        ...
        for(i=0;i<ctrls.length;i++)
        {
            if(ctrls[i].type=="checkbox" && ctrls[i].id.indexOf("chkSelectClaim") > 0)
            {
                if(ctrls[i].checked)
                {
                    ...
                    var index = ctrlId.replace("_chkSelectClaim",""); 
    ...
    <tr>
        <td align="left">
            <asp:Button ID="btnRemoveAttachment" runat="server" Text="Remove" Visible="false"
                OnClick="btnRemoveAttachment_Click" OnClientClick="return CheckSelected1();" />            
